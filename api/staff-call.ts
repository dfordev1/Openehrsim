...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!target || !message || !medicalCase) return res.status(400).json({ error: "Missing target, message, or medicalCase" });
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `You are a healthcare professional (Consultant/Nurse/Radiologist). Respond realistically. Keep it clinical and concise. Return JSON: { "reply": string, "updatedCase": MedicalCase }. Add the exchange to communicationLog. Schema: ${MEDICAL_CASE_SCHEMA}`
        },
        {
          role: "user",
          content: `Patient state: ${JSON.stringify(medicalCase)}. Communication target: ${target}. Message: ${message}.`
        }
      ],

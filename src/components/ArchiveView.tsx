...
export function ArchiveView({ user }: ArchiveViewProps) {
  ...
  return (
    <div className="flex flex-col">
      {/* ── Summary bar ──────────────────────────────────────────────────── */}
      <div className="px-4 py-3 border-b border-clinical-line bg-clinical-bg/40 flex flex-wrap items-center gap-4">
          ...
          {(['table', 'summary'] as const).map(v => (
            <button
              ...
              className={cn(
                ...
                view === v ? 'bg-clinical-blue text-white' : 'text-clinical-slate hover:bg-clinical-bg'
              )}
            >{v}</button>
          ))}
        </div>

...
export function DiagnosisPad({
  ...
  if (!isOpen) {
    if (isMobile) {
      return (
        <button
          onClick={onToggle}
          className="fixed bottom-20 right-4 z-50 bg-clinical-teal hover:bg-clinical-teal-strong text-white w-14 h-14 rounded-full shadow-xl flex items-center justify-center transition-colors"
    ...
    return (
      <button
        onClick={onToggle}
        className="fixed right-0 top-1/2 -translate-y-1/2 z-50 bg-clinical-teal hover:bg-clinical-teal-strong text-white px-2 py-4 rounded-l-lg shadow-lg transition-colors flex flex-col items-center gap-1"
        aria-label="Open Diagnosis Pad"
      >
    ...
    <div className="flex-1 overflow-y-auto p-4">
      ...
      {activeTab === 'ddx' && (
        <div className="space-y-3 animate-in fade-in">
          <label className="text-[10px] font-bold text-clinical-teal uppercase tracking-wide">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2.5 bg-clinical-bg border border-clinical-line rounded-lg text-xs focus:outline-none focus:border-clinical-teal focus:ring-1 focus:ring-clinical-teal/30"
              ...
              >
                {filteredDiagnoses.map(dx => (
                  <button
                    ...
                    className="w-full text-left px-3 py-2 text-xs hover:bg-clinical-teal-soft/50 text-clinical-ink transition-colors flex items-center gap-2 border-b border-clinical-line/50 last:border-b-0"
                  >
          ...
          {ddxSearch.trim().length >= 3 && filteredDiagnoses.length === 0 && (
            <button
              ...
              className="w-full text-left px-3 py-2.5 text-xs bg-clinical-teal-soft border border-clinical-teal/30 rounded-lg text-clinical-teal font-medium hover:bg-clinical-teal-soft/70 transition-colors flex items-center gap-2"
            >
              ...
              differentials.map((entry, idx) => (
                <div
                  ...
                  className={cn(
                    ...
                    entry.isLead
                      ? 'bg-clinical-teal-soft/60 border-clinical-teal/40 ring-1 ring-clinical-teal/30'
                  ...
                  <div className="flex items-center gap-2 px-3 py-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      <p
                        className={cn(
                          ...
                          entry.isLead && 'text-clinical-teal',
                        )}
                      >
                        {entry.diagnosis}
                      </p>
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        entry.isLead
                          ? 'text-clinical-teal'
                          : 'text-clinical-slate/30 hover:text-clinical-teal',
                      )}
                    ...
                    <button
                      ...
                      className="p-1 text-clinical-slate/30 hover:text-clinical-red transition-colors"
                      aria-label="Remove"
                    >
      ...
      {activeTab === 'findings' && (
        <div className="space-y-3 animate-in fade-in">
          <div className="flex items-center justify-between">
            <label className="text-[10px] font-bold text-clinical-teal uppercase tracking-wide flex items-center gap-1.5">
              ...
              {findings.map(f => {
                ...
                return (
                  ...
                  >
                    ...
                    {linkedCount > 0 && (
                      <span
                        className="text-[9px] font-medium text-clinical-teal bg-clinical-teal-soft rounded px-1.5 py-0.5"
                    ...
                    <button
                      ...
                      className="p-0.5 text-clinical-slate/30 hover:text-clinical-red transition-colors"
          ...
          {findings.length > 0 && (
            <div className="flex flex-wrap gap-3 pt-2 border-t border-clinical-line/50">
              {[
                { color: 'bg-clinical-red', label: 'Vitals' },
                { color: 'bg-blue-400', label: 'Exam' },
                { color: 'bg-clinical-green', label: 'Lab' },
          ...
          {findings.length > 0 && differentials.length > 0 && (
            <button
              ...
              className="mt-2 w-full text-[10px] font-medium text-clinical-teal bg-clinical-teal-soft border border-clinical-teal/30 rounded-md py-1.5 hover:bg-clinical-teal-soft/70 transition-colors flex items-center justify-center gap-1.5"
            >
      ...
      {tabs.map(tab => (
        <button
          ...
          className={cn(
            ...
            activeTab === tab.id
              ? 'text-clinical-teal border-b-2 border-clinical-teal bg-clinical-teal-soft/30'
    ...
    return (
      <>
        ...
        <motion.div
          ...
          style={{ height }}
          className="fixed left-0 right-0 bottom-0 bg-clinical-surface rounded-t-2xl shadow-2xl z-50 flex flex-col lg:hidden border-t-2 border-clinical-teal"
          role="dialog"
          aria-modal="true"
        ...
        >
          ...
          <div className="h-10 bg-clinical-teal text-white px-4 flex items-center justify-between shrink-0">
            ...
            <button
              onClick={onToggle}
              className="p-1.5 hover:bg-clinical-teal-strong rounded-md transition-colors"
              aria-label="Close"
            >
  ...
  return (
    <motion.div
      ...
      className="fixed right-0 top-0 bottom-0 w-[360px] bg-clinical-surface border-l-2 border-clinical-teal shadow-2xl z-50 flex-col hidden lg:flex"
      role="complementary"
      aria-label="Diagnosis Pad"
    >
      <div className="h-12 bg-clinical-teal text-white px-4 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2">
          <Lightbulb className="w-4 h-4" />
          <span className="text-sm font-bold tracking-wide">DIAGNOSIS PAD</span>
        </div>
    ...
    <div className="space-y-3 animate-in fade-in">
      <div className="flex items-center justify-between">
        ...
        {prIsDirty && hasHistory && (
          <span className="text-[9px] font-medium text-clinical-amber bg-clinical-amber/10 border border-clinical-amber/20 rounded-full px-2 py-0.5">
            Uncommitted
          </span>
        )}
      </div>
      ...
      <textarea
        ...
        className="w-full h-32 bg-clinical-bg border border-clinical-line rounded-lg p-3 text-sm leading-relaxed focus:outline-none focus:border-clinical-teal focus:ring-1 focus:ring-clinical-teal/30 resize-none"

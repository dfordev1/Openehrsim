...
const CATEGORY_COLOR: Record<string, string> = {
  lab:       'bg-clinical-blue-soft text-clinical-blue',
  imaging:   'bg-purple-500/10 text-purple-400',
  medication:'bg-clinical-green-soft text-clinical-green',
  consult:   'bg-clinical-amber-soft text-clinical-amber',
  procedure: 'bg-clinical-line text-clinical-slate',
};
...
export function OrderBar({
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 order-bar">
      ...
      <AnimatePresence>
        {suggestions.length > 0 && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 4 }}
            className="border-t border-clinical-line max-h-56 overflow-y-auto" style={{ background: 'var(--clinical-surface)' }}
          >
            {suggestions.map(r => (
              <button
                key={r.name}
                ...
                disabled={busy}
                className="w-full text-left px-4 py-2.5 flex items-center gap-3 hover:bg-clinical-line/50 border-b border-clinical-line/50 last:border-0 transition-colors"
              >
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-clinical-ink">{r.name}</p>
                  {(r.route || r.frequency) && (
                    <p className="text-[10px] text-clinical-slate">{[r.route, r.frequency].filter(Boolean).join(' · ')}</p>
                  )}
                </div>
      ...
      <AnimatePresence>
        {transferOpen && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 4 }}
            className="flex gap-5 px-4 py-2.5 border-t border-clinical-line"
          >
            {['ICU', 'OR', 'Cath Lab', 'Ward', 'Radiology'].map(dept => (
              <button
                ...
                disabled={busy}
                className="text-xs text-clinical-slate hover:text-clinical-ink transition-colors disabled:opacity-30"
              >
                {dept}
              </button>
            ))}
      ...
      {/* Time-pressure hint */}
      {penaltyLevel && (
        <p className={cn('px-4 py-1 text-[10px] border-t border-clinical-line/50',
          penaltyLevel === 'low' ? 'text-clinical-amber' :
          penaltyLevel === 'moderate' ? 'text-orange-400' : 'text-clinical-red font-medium'
        )}>
      ...
      {/* Main input row */}
      <div className="border-t border-clinical-line px-4 pt-2.5 pb-safe-or-3">
        <div className="flex items-center gap-2">
          <input
            ...
            className="flex-1 text-sm py-1 focus:outline-none bg-transparent text-clinical-ink placeholder-clinical-slate/40 disabled:opacity-50"
            autoComplete="off"
          />
          {value.trim() && !busy && (
            <button
              onClick={executeFreetextOrder}
              className="shrink-0 px-3 py-1.5 bg-clinical-teal text-white text-xs font-medium rounded-full hover:opacity-90 transition-opacity"
            >
              Execute
            </button>
          )}
        ...
        <div className="flex items-center gap-4 pt-1.5 pb-1">
          ...
          <button
            ...
            disabled={busy || !medicalCase}
            className={cn('text-xs transition-colors disabled:opacity-30', transferOpen ? 'text-clinical-ink' : 'text-clinical-slate hover:text-clinical-ink')}
          >
            Transfer
          </button>
          <button
          ...
          <button
            ...
            className="text-xs text-clinical-slate hover:text-clinical-ink transition-colors disabled:opacity-30 ml-auto"
          >
            End case →
          </button>
        </div>

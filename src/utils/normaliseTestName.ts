...
// Reverse map: lowercase(canonical) → proper-case canonical, built once at module load.
// Lets any canonical name (CBC, Lactate, CT Head…) round-trip regardless of input case.
const CANONICAL: Record<string, string> = {};
for (const v of Object.values(ALIASES)) {
  CANONICAL[v.toLowerCase()] = v;
}
...
export function normaliseTestName(raw: string): string {
  ...
  return ALIASES[lower] ?? CANONICAL[lower] ?? raw.trim();
}
